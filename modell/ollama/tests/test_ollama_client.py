...
from modell.ollama.src.mcp_ollama_python.models import ChatMessage, GenerationOptions, MessageRole, NetworkError
    ...
    def test_environment_host(self):
        ...
        with patch.dict(os.environ, {"OLLAMA_HOST": "http://env-host:11434"}):
            with patch('httpx.AsyncClient') as mock_client:
                ...
                import importlib
                from modell.ollama.src.mcp_ollama_python import ollama_client
...
class TestOllamaClientGenerate:
    ...
    async def test_generate_basic(self, mock_ollama_response_generate):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            assert call_args.kwargs["json"]["options"]["num_predict"] == 25
    ...
    @pytest.mark.asyncio
    async def test_generate_with_explicit_num_predict(self, mock_ollama_response_generate):
        """Test generation preserves explicit num_predict"""
        with patch('httpx.AsyncClient') as mock_client_class:
            mock_response = MagicMock()
            mock_response.json.return_value = mock_ollama_response_generate
            mock_response.raise_for_status = MagicMock()
            ...
            options = GenerationOptions(num_predict=10)
            await client.generate("llama3.1", "Hello", options=options)

            call_args = mock_client.post.call_args
            assert call_args.kwargs["json"]["options"]["num_predict"] == 10
